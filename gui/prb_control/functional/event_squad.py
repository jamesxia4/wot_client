#Embedded file name: scripts/client/gui/prb_control/functional/event_squad.py
import BigWorld
import constants
...
from gui.prb_control.restrictions.limits import EventSquadLimits
...
class EventSquadEntry(PrbEntry):

    def makeDefCtx(self):
        return prb_ctx.EventSquadSettingsCtx('prebattle/create')
...
class EventSquadFunctional(PrbFunctional):
    SLOTS_COUNT = 5
    ...
    def __init__(self, settings):
        ...
        super(EventSquadFunctional, self).__init__(settings, permClass=SquadPrbPermissions, requestHandlers=requests, limits=EventSquadLimits(self))
    ...
    def setPlayerState(self, ctx, callback = None):
        super(EventSquadFunctional, self).setPlayerState(ctx, callback)

    def showGUI(self):
        g_eventDispatcher.loadEventSquad()
    ...
    def doSelectAction(self, action):
        result = False
        if action.actionName == PREBATTLE_ACTION_NAME.EVENT_SQUAD:
            g_eventDispatcher.showEventSquadWindow()
            result = True
    ...
    def kickPlayer(self, ctx, callback = None):
        ...
        super(EventSquadFunctional, self).kickPlayer(prb_ctx.KickPlayerCtx(playerInfo.accID), ctx.getWaitingID())
    ...
    def prb_onPlayerRosterChanged(self, pID, prevRoster, roster, actorID):
        super(EventSquadFunctional, self).prb_onPlayerRosterChanged(pID, prevRoster, roster, actorID)
        g_eventDispatcher.updateUI()

    def prb_onPlayerRemoved(self, pID, roster, name):
        super(EventSquadFunctional, self).prb_onPlayerRemoved(pID, roster, name)
        g_eventDispatcher.updateUI()
        g_eventDispatcher.setEventSquadTeamReadyInCarousel(self.getPrbType(), self.__isTeamRead())

    def prb_onPlayerAdded(self, pID, roster):
        super(EventSquadFunctional, self).prb_onPlayerAdded(pID, roster)
        g_eventDispatcher.setEventSquadTeamReadyInCarousel(self.getPrbType(), self.__isTeamRead())
    ...
    def __handleCarouselInited(self, _):
        g_eventDispatcher.addEventSquadToCarousel(self.__isTeamRead())
