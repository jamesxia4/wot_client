...
class FittingItem(object):
    ...
    def __cmp__(self, other):
        ...
        if self.level > other.level:
            return 1
        if self.priceOrder[1] < other.priceOrder[1]:
            return -1
        if self.priceOrder[1] > other.priceOrder[1]:
            return 1
        if self.priceOrder[0] < other.priceOrder[0]:
            return -1
        if self.priceOrder[0] > other.priceOrder[0]:
            return 1
...
class InventoryTankman(InventoryItem):
    ...
    def addSkill(self, skillName, callback):
        ...
        if ready and skillName not in self.skills:
...
class VehicleItem(FittingItem):
    ...
    def __init__(self, descriptor = None, compactDescr = None, count = 0):
        assert descriptor is not None or compactDescr is not None, 'descriptor or compactDescr should be provided for VehicleItem'
        self.count = count
        self.descriptor = descriptor
        self.vehicles = []
        if descriptor is None:
            self.descriptor = getDictDescr(compactDescr)
